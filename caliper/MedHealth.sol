...
contract MedHealth {
    ...
    // 更新药品代购凭证
    function updateDrugDelivery(uint id,string memory certIHash) public  {
        // 查询代购信息
        DrugDelivery storage drugDelivery = drugDeliveryMapping[id];
        // 首先查询药品信息是否存在
        require(drugDelivery.id != 0, "药品代买信息不存在");
        // 对用户的身份进行验证，判断是否为医师用户
        require(keccak256(abi.encodePacked(accountMapping[msg.sender].role)) == keccak256(abi.encodePacked("医师")), "此操作(登记药品代买)只能患者使用");
        // 判断是否已经完成审核
        require(drugDelivery.isReview == 0,"该记录已经被审核，无法修改凭证");
        // 更新派送凭证信息
        drugDelivery.certIHash = certIHash;
    }
}
