...
contract MedHealth {
    ...
    struct Inquiry {
        ...
        string inqueryType; //问诊类型: 在加就医/社区就医
    ...
    //问诊操作
    //患者提交问诊
    function patientRegistrationInquiry(uint id, string inqueryType) public {
        ...
        inquiryMapping[id] = Inquiry(id, msg.sender, 0x0, 0x0, inqueryType, "","", 0,0,0,0);
    }
    ...
    function getInquiry(uint id)public view returns(uint, address, address, address, string, string, string, uint8, uint8, uint8,uint8){
        require(inquiryMapping[id].id != 0, "问诊信息不存在");
        Inquiry inquiry = inquiryMapping[id];
        return (inquiry.id, inquiry.patientAddress, inquiry.physicianAddress, inquiry.supervisorAddress, inquiry.inqueryType, inquiry.medicalHash, inquiry. videoHash, inquiry.isDis, inquiry.isReception, inquiry.isDiagnosis, inquiry.isReview);
    }
