	...
	<view class="container">
		...
		<view class="main-container">
					...
					<view class="info-section">
						...
						<view class="card-body">
							...
							<view class="item">
								...
								<text class="value">{{ item.patient.username }}</text>
							</view>
							<view class="item">
								<text class="label">年龄:</text>
								<text class="value">{{ item.patient.age }}</text>
							</view>
							<view class="item">
								<text class="label">性别:</text>
								<text class="value">{{ item.patient.sex }}</text>
							</view>
							<view class="item">
			...
			<uni-fab :pattern="pattern" :horizontal="horizontal" :vertical="vertical" :direction="direction"
				@fabClick="fabClick" v-if="$store.state.user.userInfo.role === '患者'"></uni-fab>
		</view>
	</view>
</template>
...
<script>
	import {
		queryPhysicianInquiryRecord
	} from '../api/system/doctor'
	import {
		queryPatientInquiryRecord
	...
	export default {
		data() {
			return {
				...
				activeColor: '#007aff',
				styleType: 'button',
				appointmentTime: '2024-10-08 10:00',
				name: '张三',
				age: '30',
				gender: '男',
				consultationType: '内科'
		...
		methods: {
			...
			async tabClick(e) {
				...
				if (e.name === '预约信息') {
					if (this.$store.state.user.userInfo.role === '患者') {
						const res = await queryPatientInquiryRecord('2')
						this.infoList = res.data[0].list
						console.log(this.infoList)
					} else if (this.$store.state.user.userInfo.role === '医师') {
						const res = await queryPhysicianInquiryRecord('4')
						this.infoList = res.data[0].list
						console.log(this.infoList)
					}
				} else if (e.name === '待处理问诊') {
					const res = await queryPhysicianInquiryRecord('2')
					this.infoList = res.data[0].list
					console.log(this.infoList)
				} else if (e.name === '历史问诊') {
					if (this.$store.state.user.userInfo.role === '患者') {
						const res = await queryPatientInquiryRecord('1')
						this.infoList = res.data[0].list
						console.log(this.infoList)
					} else if (this.$store.state.user.userInfo.role === '医师') {
						const res = await queryPhysicianInquiryRecord('1')
						this.infoList = res.data[0].list
						console.log(this.infoList)
					}
				} else {
			...
			fabClick() {
				this.$tab.navigateTo('/pages/registration/registration')
			},
			toPage(item) {
				if (this.activeIndex === '预约信息') {
					this.$tab.navigateTo(`/pages/receive/receive?id=${item.id}`)
				} else if (this.activeIndex === '待处理问诊') {
					this.$tab.navigateTo(`/pages/visit/visit?id=${item.id}`)
				} else if (this.activeIndex === '历史问诊') {
					this.$tab.navigateTo(`/pages/record/record?id=${item.id}`)
				}
			},
			logout() {
				setToken('')
